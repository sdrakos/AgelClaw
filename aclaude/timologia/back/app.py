...
async def create_company(req: CompanyReq, user=Depends(get_current_user)):
    ...
    with get_db() as conn:
        existing = conn.execute("SELECT id, name FROM companies WHERE afm = ?", (req.afm,)).fetchone()
        if existing:
            ...
            if member:
                raise HTTPException(400, "Έχετε ήδη πρόσβαση σε αυτή την εταιρεία")
            # Add user as owner to existing company
            conn.execute(
                "INSERT INTO company_members (user_id, company_id, role) VALUES (?, ?, 'owner')",
                (user["id"], existing["id"]),
            )
            return {"id": existing["id"], "name": existing["name"], "afm": req.afm}
        cur = conn.execute(
