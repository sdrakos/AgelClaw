...
const iconMap: Record<string, JSX.Element> = {
  ...
  code: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.25 6.75L22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3l-4.5 16.5" /></svg>,
  ...
  send: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" /></svg>,
  ...
  zap: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>,
  layers: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6.429 9.75L2.25 12l4.179 2.25m0-4.5l5.571 3 5.571-3m-11.142 0L2.25 7.5 12 2.25l9.75 5.25-4.179 2.25m0 0L12 12.75 6.43 9.75m11.14 0l4.179 2.25L12 17.25 2.25 12l4.179-2.25m11.142 0l4.179 2.25-4.179 2.25M6.43 14.25L2.25 16.5 12 21.75l9.75-5.25-4.179-2.25" /></svg>,
  inbox: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M2.25 13.5h3.86a2.25 2.25 0 012.012 1.244l.256.512a2.25 2.25 0 002.013 1.244h3.218a2.25 2.25 0 002.013-1.244l.256-.512a2.25 2.25 0 012.013-1.244h3.859m-17.5 0V6.108c0-1.135.845-2.098 1.976-2.192a48.424 48.424 0 0113.548 0c1.131.094 1.976 1.057 1.976 2.192V13.5" /></svg>,
  ...
  wrench: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.42 15.17l-5.66 5.66a2.12 2.12 0 01-3-3l5.66-5.66m0 0L4.93 9.68a2.12 2.12 0 010-3l1.41-1.41a2.12 2.12 0 013 0l2.49 2.49m2.59 2.59l5.66-5.66a2.12 2.12 0 013 3l-5.66 5.66m0 0l2.49 2.49a2.12 2.12 0 010 3l-1.41 1.41a2.12 2.12 0 01-3 0l-2.49-2.49" /></svg>,
...
function SkillCard({ skill }: { skill: Skill }) {
  const tierColors: Record<string, { bg: string; text: string; label: string }> = {
    free: { bg: 'bg-green-500/10', text: 'text-green-400', label: 'FREE' },
    pro: { bg: 'bg-amber-500/10', text: 'text-amber-400', label: 'PRO' },
    enterprise: { bg: 'bg-purple-500/10', text: 'text-purple-400', label: 'ENTERPRISE' },
  }

  const tier = tierColors[skill.tier] || tierColors.free

  return (
    <div className="p-5 bg-gray-900/50 border border-gray-800 rounded-xl hover:border-gray-700 transition-colors group">
      <div className="flex items-start gap-4">
        <div className="w-10 h-10 bg-indigo-500/10 border border-indigo-500/20 rounded-lg flex items-center justify-center text-indigo-400 flex-shrink-0 group-hover:bg-indigo-500/20 transition-colors">
          {iconMap[skill.icon] || <span className="text-sm">?</span>}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-sm font-semibold text-white">{skill.name}</h3>
            <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded ${tier.bg} ${tier.text}`}>
              {tier.label}
            </span>
          </div>
          <p className="text-xs text-gray-400 leading-relaxed mb-3">{skill.description}</p>
          <div className="flex items-center justify-between">
            <span className="text-[10px] uppercase tracking-wider text-gray-500 font-medium">{skill.category}</span>
            {skill.tier === 'free' ? (
              <button className="text-xs text-indigo-400 hover:text-indigo-300 font-medium transition-colors">
                Download
              </button>
            ) : (
              <span className="text-xs text-gray-500">{tier.label} only</span>
            )}
          </div>
        </div>
      </div>
