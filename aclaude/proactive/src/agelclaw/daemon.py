...
from agelclaw.agent_config import get_agent, get_system_prompt, AGENT_TOOLS, ALLOWED_TOOLS, _scan_mcp_servers, _build_mcp_tool_wildcards, load_mcp_server_config, safe_system_prompt
...
def _load_daemon_extensions() -> str:
    """Load daemon-specific instructions from persona/DAEMON.md.
    Falls back to bundled template if persona file doesn't exist."""
    persona_file = get_persona_dir() / "DAEMON.md"
    if persona_file.exists():
        try:
            content = persona_file.read_text(encoding="utf-8", errors="replace").strip()
            if content:
                return "\n\n" + content + "\n\n"
        except Exception:
            pass
    ...
    from agelclaw.project import get_package_data_dir
    bundled = get_package_data_dir() / "templates" / "DAEMON.md"
    if bundled.exists():
        try:
...
def _get_daemon_prompt() -> str:
    ...
    return get_system_prompt() + _load_daemon_extensions() + memory.build_rules_prompt()
        ...
        try:
            if route.provider == Provider.OPENAI:
                ...
                options = ClaudeAgentOptions(
                    system_prompt=safe_system_prompt(_get_daemon_prompt(), task_id=task_id),
            ...
            # Clean up per-task system prompt file
            _cleanup_prompt_file(task_id)


def _cleanup_prompt_file(task_id):
    """Remove per-task SYSTEM_PROMPT_<id>.md file after task finishes."""
    try:
        prompt_file = get_persona_dir() / f"SYSTEM_PROMPT_{task_id}.md"
    ...
    async with task_semaphore:
        ...
        try:
            if route.provider == Provider.OPENAI:
                ...
                options = ClaudeAgentOptions(
                    system_prompt=safe_system_prompt(enriched_body, task_id=task_id),
                    allowed_tools=allowed,
                    permission_mode="bypassPermissions",
                    cwd=str(proactive_dir),
...
async def _maybe_run_heartbeat():
    ...
    try:
        ...
        if route.provider == Provider.OPENAI:
            ...
            options = ClaudeAgentOptions(
                system_prompt=safe_system_prompt(_get_daemon_prompt(), task_id="heartbeat"),
